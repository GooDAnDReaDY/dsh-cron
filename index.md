...
- **Пакет**: `@goodandready/dsh-cron`, версия `0.2.4` (см. `package.json`).
- **Статус**: active. Релиз `0.2.4` — рантаймы исполнения (#4–#10, #29, #31,
  #38) и доставка с секретами (#51, #25, #26, #20–#23, #47, #28) плюс
  исправление хранилища (#112). Код смёржен в `main` (PR #114, независимое
  ревью — PASS на `7216b3a`), выпуск `0.2.4` публикуется владельцем
  одобренным циклом. Проверено: 112/112 тестов, гейт размера 22 файла
  ≤ 256 KiB, изолированный MiniPC (7 каналов на заглушку, обрыв зависшего
  канала по дедлайну, маскирование секретов, хранилище внутри `DSH_HOME`).
