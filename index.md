...
- **Статус**: active, релиз `0.2.7` готовится (блок C, операционный): задачи из
  конфига профиля (#50), внешний REST API `/dsh-cron/api/*` под Bearer-токеном
  (#54), метрики Prometheus `GET /dsh-cron/metrics` (#53), строгая проверка
  идентификаторов каналов на входе API (#121), аутентифицированная проверка
  после установки в `deploy.sh` (#126) и остаток разбивки разбора расписания и
  постановки задач (#97). Код блока готов в ветке `feat/0.2.7-ops-block`, полный
  прогон 220/220 после правок по итогам первого раунда независимого ревью; впереди повторное ревью, изолированный цикл на тестовом
  сервере, production-приёмка кандидата и вопрос владельцу о публикации.
- **Предыдущий релиз**: `0.2.6` выпущен 2026-09-11 — блок B: каскад моделей
  (#45), правило тишины (#44), диагностика сбоев (#43), каталог рецептов
  (#48), инструменты `cron_get_task`/`cron_update_task` (#49) и разбивка
  обработчиков (#97). Код — PR #124 (`9d5e25b`), независимое ревью в два раунда
  (FAIL с исправлением гейта подтверждения, PASS на `45fb85c`). Релиз `0.2.5`
  опубликован и стоит в production.
...
- Открытые долги: #117 (мажор croner 9 → 10), #113 (healthcheck тест-хелпера),
  #127 (обезличивание внутренних документов), #128 (публикационный слой для GitHub).
