...
- **Статус**: active, релиз `0.2.7` выпускается 2026-09-11 — блок C (операционный): задачи из
  конфига профиля (#50), внешний REST API `/dsh-cron/api/*` под Bearer-токеном (#54), метрики
  Prometheus `GET /dsh-cron/metrics` (#53), строгая проверка идентификаторов каналов (#121),
  аутентифицированная проверка после установки в `deploy.sh` (#126), остаток разбивки разбора
  расписания и постановки задач (#97), публикационный слой GitHub (ADR-0003, #128). Код —
  PR #130 (`31fcc32`), независимое ревью PASS (три раунда), 221/221 тестов, изолированный цикл
  на тестовом сервере и production-приёмка кандидата пройдены.
