...
- **Пакет**: `@goodandready/dsh-cron`, версия `0.2.8` (см. `package.json`).
- **Статус**: active, релиз `0.2.8` выпускается 2026-09-11 — мажорный апгрейд
  ядра планирования croner `9.1.0 → 10.0.1` (#117): числовые шаги `0/N` из
  croner 9 поддержаны через `sloppyRanges`, регрессионный тест на вооружение
  такой задачи, обновлён lockfile и deploy.sh. 222/222 тестов, `pnpm audit`
  чист, независимое ревью PASS, на тестовом сервере реальная задача по
  расписанию выполнилась дважды, production-приёмка кандидата пройдена
  (собственная задача `0 */6 * * *` вооружена croner 10). Код — PR #132
  (`fe36039`).

- **Предыдущий релиз**: `0.2.7` выпущен 2026-09-11 — блок C (операционный):
  задачи из конфига профиля (#50), внешний REST API `/dsh-cron/api/*` под
  Bearer-токеном (#54), метрики Prometheus (#53), строгая проверка
  идентификаторов каналов (#121), проверка после установки в `deploy.sh`
  (#126), разбивка парсера расписания и постановки задач (#97),
  публикационный слой GitHub (ADR-0003, #128). Код — PR #130 (`31fcc32`),
  независимое ревью PASS (три раунда), изолированный цикл и
  production-приёмка кандидата пройдены. Ранее — `0.2.6` (блок B: каскад
  моделей, правило тишины, диагностика сбоев, рецепты, инструменты и
  разбивка обработчиков).
