...
- Текущий статус: active, релиз `0.2.8` (croner 9 → 10, #117: ядро планирования на croner 10, хранимые расписания с числовыми шагами поддержаны через `sloppyRanges`) выпускается 2026-09-11 после прогона 222/222, независимого ревью (PASS) и живой проверки задачи по расписанию на тестовом сервере; код — PR #132 (`fe36039`). Предыдущий релиз `0.2.7` (блок C: операционный — задачи из конфига, внешний REST API, метрики, строгая валидация каналов, deploy verify, остаток разбивки) выпущен 2026-09-11 после прогона 221/221 и независимого ревью (PASS, три раунда); код — PR #130 (`31fcc32`), публикационный слой — ADR-0003 (#128). Ранее — `0.2.6` (блок B: экономия и меньше шума, плюс разбивка обработчиков) выпущен 2026-09-11 после прогона 175/175 и независимого ревью (PASS на `45fb85c`); код — PR #124 (`9d5e25b`). Ранее — `0.2.5` (блок A пользовательских поверхностей
