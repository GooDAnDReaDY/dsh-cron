...
- Текущий статус: active. Блок «рантаймы + доставка/секреты» смёржен в `main`
  (PR #114, merge `59f6f8d`, независимое ревью — PASS на `7216b3a`) и проверен
  на MiniPC; в production-профиле MiniAI установлена `0.2.3` (совпадает с
  `npm latest`), релиз `0.2.4` ждёт явного «Публикуем релиз?»
