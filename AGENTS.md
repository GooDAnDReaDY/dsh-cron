...
- Текущий статус: active, релиз `0.2.6` (блок B: экономия и меньше шума, плюс разбивка обработчиков) выпускается 2026-09-11 после прогона 175/175 и независимого ревью (PASS на `45fb85c`); код — PR #124 (`9d5e25b`). Предыдущий релиз `0.2.5` (блок A пользовательских поверхностей
...
- Текущая версия: `0.2.6` (package.json). Обычный релиз меняет только `z`;
