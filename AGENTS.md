...
- Текущий статус: active, релиз `0.2.4` (рантаймы исполнения и доставка с
  секретами) выпущен 2026-09-11 после полного прогона 112/112, приёмки на
  MiniPC и независимого ревью (PASS на `7216b3a`). Код — PR #114, релиз —
  PR #118 (`e2e5125`); пакет опубликован в npm как `latest`, в production
  web-профиле MiniAI стоит точная версия `0.2.4` с пройденными
  health/smoke-проверками.
...
- Текущая версия: `0.2.4` (package.json). Обычный релиз меняет только `z`;
