...
- Текущий статус: active, релиз `0.2.7` (блок C: операционный — задачи из конфига, внешний REST API, метрики, строгая валидация каналов, deploy verify, остаток разбивки) выпускается 2026-09-11 после прогона 221/221 и независимого ревью (PASS, три раунда); код — PR #130 (`31fcc32`), публикационный слой — ADR-0003 (#128). Предыдущий релиз `0.2.6` (блок B: экономия и меньше шума, плюс разбивка обработчиков) выпущен 2026-09-11 после прогона 175/175 и независимого ревью (PASS на `45fb85c`); код — PR #124 (`9d5e25b`). Ранее — `0.2.5` (блок A пользовательских поверхностей
...
- Текущая версия: `0.2.7` (package.json). Обычный релиз меняет только `z`;
