...
export function parseLlmActionDirectives(text) {
  ...
  while ((match = blockRegex.exec(text)) !== null) {
    ...
    if ((raw.startsWith('{') && raw.endsWith('}')) || (raw.startsWith('[') && raw.endsWith(']'))) {
      bestEffort('parse-directive-json', () => {
  ...
  if (directives.length === 0) {
    ...
    while ((im = inlineMatch.exec(text)) !== null) {
      bestEffort('parse-inline-directive', () => {
...
async function tryCreateGiteaIssue({ title, body, repo = 'goodandready/dsh-cron', fetchFn }) {
  try {
    ...
    const credPath = process.env.GITEA_CREDENTIALS_PATH || '';
    ...
    /* ignore Gitea creation errors, best-effort */
  }
  return null;
}
