...
import { rejectCrossOrigin } from './http-utils.js';
...
function profileNameFromArgv(argv) {
  for (let index = 2; index < argv.length; index += 1) {
    if (argv[index] === '--profile' || argv[index] === '-p') return argv[index + 1];
...
export async function installExact(target, packageSpec, options = {}) {
  ...
  return new Promise((resolvePromise, reject) => {
    // IMPORTANT: As required by #147, do NOT add minimumReleaseAge bypass
    const args = [
...
export function registerPluginUpdater(ctx, options) {
  ...
  return ctx.webServer.register({
    kind: 'exact',
    path: options.endpoint || '/api/dsh-cron/update',
    handler: async (req, res) => {
      try {
        ...
        (ctx.logger || console).warn?.(`[dsh-cron] plugin updater failed: ${String(error)}`);
        res.writeHead(503, { 'content-type': 'application/json; charset=utf-8' });
