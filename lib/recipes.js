...
export function listRecipes() {
  const destructive = findDestructiveRecipe();
  const safeRecipes = destructive ? RECIPES.filter((r) => r.id !== destructive.id) : RECIPES;
  return safeRecipes.map((recipe) => ({ ...recipe, channels: [...(recipe.channels || [])] }));
}
