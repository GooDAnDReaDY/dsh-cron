...
import { applyTaskPatch, isCodeTypeSwitch } from './task-patch.js';
...
async function patchTask({ store, scheduler, req, res, id }) {
  ...
  const { body, error } = await readBody(req, res);
  if (error) return;
  // Switching a task into a code-executing type over HTTP needs the same
  // explicit confirmation as creating one (#86).
  if (isCodeTypeSwitch(current, body && body.type) && req.headers[SCRIPT_CONFIRM_HEADER] !== 'script') {
    sendJson(res, 403, {
      ok: false,
      error: `Switching a task to the ${normalizeTaskType(body.type)} type over HTTP requires the ${SCRIPT_CONFIRM_HEADER}: script header`,
    });
    return;
  }
  const result = applyTaskPatch({ store, scheduler, id, body });
  if (!result.ok) {
    sendJson(res, result.notFound ? 404 : 400, { ok: false, error: result.error });
    return;
  }
