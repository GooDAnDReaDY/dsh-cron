...
function refuseConfigOwned(res, id) {
  sendJson(res, 409, {
    ok: false,
    error: 'Task "' + id + '" is declared in the profile config (config.jobs); change the config file instead',
  });
}
...
export async function handleExternalTaskRequest({ store, scheduler, req, res, url }) {
  const parts = url.pathname.split('/').filter(Boolean); // dsh-cron, api, tasks, :id?, :action?
  const id = parts[3];
