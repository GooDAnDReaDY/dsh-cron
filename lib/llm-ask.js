...
export function resolveAskTarget(ctx, task = {}, preferredModel = '') {
  ...
  defaultSelection = bestEffort('resolve-default-model', () => {
...
async function buildUserMessage(text) {
  ...
  const source = { kind: 'plugin:dsh-cron', form: 'helper-call' };
  const mod = await bestEffort('import-dsh-llm', () => import('@deepseek-ai/dsh-llm'));
