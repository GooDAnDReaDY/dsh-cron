...
const AGENT_TASK_TYPES = ['llm', 'skill', 'workflow'];
...
export class TaskScheduler {
  ...
  async checkHeartbeats() {
    ...
    for (const task of tasks) {
      ...
      if (now > deadline && !task.heartbeatAlerted) {
        ...
        bestEffort('record-run', () => this.store.recordRun(task.id, runInfo), this.logger);
  ...
  async completeRun(task, taskId, currentRun, outcome, start, options = {}) {
    ...
    while (this.queue.length > 0 && this.running.size < this.maxConcurrent) {
      ...
      if (!nextItem) break;
      const queuedTask = this.store.get(nextItem.taskId);
    ...
    try {
      ...
      if (failed && maxRetries > 0 && (task.attempts || 0) < maxRetries && task.status !== 'paused') {
        ...
        this.retryTimers.set(taskId, setTimeout(() => {
          this.retryTimers.delete(taskId);
          this.runTask(taskId, { isRetry: true });
        }, backoffMs));
        ...
        // Retry budget exhausted: reset attempts so the next scheduled run starts fresh (#145)
        task.attempts = 0;
        this.store.set(task);
      }
    } catch (err) {
  ...
  pauseTask(taskId) {
    ...
    if (running) {
      bestEffort('abort-paused-task', () => running.controller.abort(new Error('Task paused')), this.logger);
      this.running.delete(taskId);
    }
  ...
  removeTask(taskId) {
    ...
    const running = this.running.get(taskId);
    if (running) {
      bestEffort('abort-deleted-task', () => running.controller.abort(new Error('Task deleted')), this.logger);
      this.running.delete(taskId);
    }
    this.queue = this.queue.filter((item) => item.taskId !== taskId);
  }
