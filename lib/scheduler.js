...
export function parseScheduleExpression(input) {
  ...
  // 4. Friendly recurring aliases (English plus accepted Russian input aliases)
...
export class TaskScheduler {
  ...
  start() {
    ...
    for (const task of tasks) {
      if (task.status === 'active') {
        ...
        if (task.nextRunAt && (now - task.nextRunAt) > 5 * 60 * 1000) {
          const missedAt = task.nextRunAt;
          console.warn(`[dsh-cron] Task "${task.title}" (${task.id}) missed scheduled run at ${new Date(missedAt).toISOString()}`);
  ...
  removeTask(taskId) {
    ...
    if (running) {
      try {
        running.controller.abort(new Error('Task deleted'));
      } catch {}
      this.running.delete(taskId);
    }
