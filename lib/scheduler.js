...
export class TaskScheduler {
  ...
  async executeOnce(task, signal) {
    const result = { status: 'success', output: '', error: null, usage: { inputTokens: 0, outputTokens: 0, cacheReadTokens: 0 }, costUsd: 0, sessionId: null };
  ...
  async runTask(taskId) {
    ...
    const outcome = await this.executeOnce(task, currentRun.controller.signal);
    const { status, output, error, usage, costUsd, sessionId } = outcome;
