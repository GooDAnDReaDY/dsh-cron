 ...
 */
export function renderMetrics({ tasks = [], stats = {}, runCounters = {}, scheduler } = {}) {
  const lines = [];
  const byStatus = {};
  ...
  if (scheduler && typeof scheduler.runningCount === 'function') {
    lines.push('# HELP dsh_cron_concurrent_running Number of currently executing tasks.');
    lines.push('# TYPE dsh_cron_concurrent_running gauge');
    lines.push('dsh_cron_concurrent_running ' + (Number(scheduler.runningCount()) || 0));
  }

  lines.push('# HELP dsh_cron_task_tokens_total Cumulative tokens consumed by task.');
  lines.push('# TYPE dsh_cron_task_tokens_total counter');
  for (const task of tasks) {
    if (Number(task.totalTokens) > 0) {
      lines.push(sample('dsh_cron_task_tokens_total', { task: task.id }, task.totalTokens));
    }
  }

  lines.push('# HELP dsh_cron_task_cost_usd_total Estimated cumulative LLM cost in USD.');
  lines.push('# TYPE dsh_cron_task_cost_usd_total counter');
  for (const task of tasks) {
    if (Number(task.totalCostUsd) > 0) {
      lines.push(sample('dsh_cron_task_cost_usd_total', { task: task.id }, task.totalCostUsd));
    }
  }
