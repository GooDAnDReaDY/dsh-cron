...
import { pickPatchableFields, restoreTaskSecrets } from './http-utils.js';
...
export function buildTaskPatch(current, body) {
  const mergedBody = current ? restoreTaskSecrets(body || {}, current) : (body || {});
  const patch = pickPatchableFields(mergedBody, PATCHABLE_TASK_FIELDS);
