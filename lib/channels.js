...
export const CHANNEL_HANDLERS = {
  ...
  async email({ task, runInfo, settings, deps, resolveSecret }) {
 ...
 * bounded by a timeout, so a slow endpoint cannot delay the rest of the report
 * nor hold the cron callback open. Never throws: per-channel failures are
