...
import { formatTaskTelegramMessage, sendTelegramMessage, createTelegramTaskKeyboard, shouldNotifyTask } from './telegram.js';
 ...
 * visible failure.
 */
export function unknownChannelIds(list) {
  if (!Array.isArray(list)) return [];
  return list.filter((id) => !CHANNEL_IDS.includes(id));
}

export function unknownChannels(task) {
  return unknownChannelIds(task && task.channels);
}
...
export function shouldSendToChannel(channelId, task, runInfo, settings = {}) {
  ...
  if (channelId === 'telegram') {
    if (task && task.notifyTelegram === false) return false;
    const taskForTelegram = Object.assign({}, task, { notifyTelegram: true });
    return shouldNotifyTask(taskForTelegram, runInfo, settings);
  }
...
const CHANNEL_HANDLERS = {
  async telegram({ task, runInfo, settings, secrets, http, signal, timeoutMs }) {
    ...
    try {
      const replyMarkup = createTelegramTaskKeyboard(task, runInfo);
      await sendTelegramMessage({ botToken, chatId, text, replyMarkup, fetchFn: http, signal });
    } catch (err) {
