(function () {
  ...
  function dshCronModule(require, exports, module) {
    ...
    function nativeButton() {
      const candidates = [
        'nav button[class*="sidebar-entry"]',
        'aside button[class*="sidebar-entry"]',
        '[class*="sidebar"] button[class*="sidebar-entry"]',
        'nav button',
        'aside button',
      ...
      for (const sel of candidates) {
        ...
        for (const el of found) {
          if (el.hasAttribute(ENTRY_ATTR)) continue;
          if (el.dataset && el.dataset.dshPart === 'sidebar-entry') continue;
    ...
    function placeEntry(target, entry) {
      ...
      while (next !== null && next.tagName === 'BUTTON'
        && (String(next.className).indexOf('sidebar-entry') >= 0 || next.hasAttribute('data-dsh-part'))) {
        anchor = next;
