...
const React = window.React || require('react');
    ...
    const STYLES = `
      ...
      .dsh-cron-stat-lbl { font-size: 11.5px; color: #777; margin-top: 2px; }
            .dsh-cron-oneshot-tag { font-size: 11px; padding: 2px 6px; border-radius: 4px; background: rgba(168, 85, 247, 0.15); color: #c084fc; font-weight: 500; }
      ...
      const fetchTasks = async () => {
        ...
        try {
          ...
          if (data && data.ok) {
            ...
            setFetchError(data && data.error ? data.error : 'Не удалось загрузить задачи');
          }
        } catch (err) {
          console.error('[dsh-cron] fetch tasks error:', err);
          setFetchError(err.message || 'Ошибка сети при загрузке задач');
        } finally {
          setLoading(false);
        }
      };
        ...
        try {
          ...
          if (data && data.ok) {
            if (!providerName) {
              ...
              setDefaultModelInfo(data.defaultModel || null);
      ...
      return React.createElement('div', { className: 'dsh-cron-overlay' },
        React.createElement('div', { className: 'dsh-cron-container' },
          ...
          fetchError ? React.createElement('div', {
            ...
            React.createElement('span', null, 'Ошибка загрузки задач: ' + fetchError),
            React.createElement('button', {
              type: 'button',
              className: 'dsh-cron-btn',
          ...
          React.createElement('div', { className: 'dsh-cron-task-list' },
            loading && tasks.length === 0
              ? React.createElement('div', { style: { color: '#888', padding: '16px 0', fontSize: '14px' } }, 'Загрузка запланированных задач...')
              : tasks.length === 0
                  ...
                  React.createElement('div', {
                    ...
                    React.createElement('div', { className: 'dsh-cron-task-info' },
                      React.createElement('div', { className: 'dsh-cron-task-title' },
                        ...
                        task.oneShot ? React.createElement('span', { className: 'dsh-cron-oneshot-tag' }, 'Разовая') : null,
              ...
              React.createElement('div', { className: 'dsh-cron-form-row', style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' } },
                ...
                React.createElement('div', { className: 'dsh-cron-form-group' },
                  ...
                  React.createElement('input', {
                    required: true,
                    value: formSchedule,
                    placeholder: '0 */6 * * * или every 2h или in 30m или at: 2026-09-05T15:00:00Z',
    ...
    function CronSettingsCard(props) {
      ...
      const testTelegram = async () => {
        setTestInfo({ loading: true, message: 'Отправка...' });
        try {
          const res = await fetch('/dsh-cron/telegram/test', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          ...
          const data = await res.json();
          if (data && data.ok) {
            setTestInfo({ success: true, message: '✅ Успешно доставлено в Telegram' });
          } else {
            setTestInfo({ success: false, message: '❌ ' + (data?.error || 'Сбой отправки') });
          }
        } catch (err) {
          setTestInfo({ success: false, message: '❌ ' + err.message });
        }
      };

      const testKanban = async () => {
        setTestInfo({ loading: true, message: 'Проверка Kanban...' });
        try {
          ...
          const data = await res.json();
          if (data && data.ok) {
            setTestInfo({ success: true, message: '✅ Карточка в Kanban создана' });
          } else {
            setTestInfo({ success: false, message: '❌ ' + (data?.error || 'Ошибка Kanban') });
          }
        } catch (err) {
          setTestInfo({ success: false, message: '❌ ' + err.message });
        }
      ...
      return React.createElement('div', { className: 'dsh-cron-card' },
        ...
        React.createElement('div', { className: 'dsh-cron-card-desc', style: { marginBottom: '16px' } },
          'Автоматическое выполнение задач по расписанию (cron/интервалы), фоновые сценарии агентов и интеграция с Telegram и Kanban.'
        ),
        React.createElement('form', { onSubmit: saveSettings, style: { display: 'flex', flexDirection: 'column', gap: '12px' } },
          React.createElement('div', { className: 'dsh-cron-form-group', style: { marginBottom: 0 } },
            ...
            React.createElement('input', {
              type: 'password',
              value: token,
              placeholder: hasDefault ? 'Используется бот из настроек DSH (или введите свой)' : '123456789:ABCdefGhIJKlmNoPQRsTUVwxyZ',
              onChange: (e) => setToken(e.target.value)
            }),
            hasDefault && React.createElement('div', { className: 'dsh-cron-hint' }, '✓ Найден настроенный бот в профиле DSH')
          ),
          React.createElement('div', { className: 'dsh-cron-form-group', style: { marginBottom: 0 } },
            React.createElement('label', null, 'Telegram Chat ID'),
            React.createElement('input', {
          ...
          React.createElement('div', { className: 'dsh-cron-form-group', style: { marginBottom: 0 } },
            React.createElement('label', null, 'Kanban Base URL'),
            React.createElement('input', {
              value: kanbanUrl,
          ...
          React.createElement('div', { style: { background: 'rgba(255,255,255,0.02)', border: '1px solid #2e2e2e', borderRadius: '6px', padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: '8px' } },
            React.createElement('label', { className: 'dsh-cron-checkbox-row' },
              React.createElement('input', {
                type: 'checkbox',
                checked: notify,
            ...
            React.createElement('label', { className: 'dsh-cron-checkbox-row' },
              ...
              React.createElement('span', null, 'Уведомлять только при ошибках')
            )
          ),
    ...
    function mountCronScreen(ctx, toggle) {
      ...
      const ensure = () => {
        ...
        if (ReactDOM && typeof ReactDOM.createRoot === 'function') {
          ...
          ReactDOM.render(React.createElement(CronScreen, { ctx, toggle, onClose: () => toggle.set(false) }), container);
        }
      };
