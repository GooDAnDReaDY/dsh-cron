window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const ROW_ID = 'dsh-cron';
    const ROW_CONFIG_KEY = PLUGIN_ID + '#' + ROW_ID;
    const PANEL = 'cron';
    ...
    const STRINGS = {
      en: {
        ...
        'updater.title': 'Plugin Auto-Updater',
        'updater.btnCheck': 'Check for updates',
        'updater.checking': 'Checking...',
        'updater.btnUpdate': 'Update to v{version}',
        'updater.updating': 'Updating...',
        'updater.desc': 'Check npm registry and install plugin updates in-place without SSH access.',
        'updater.current': 'Current version: v{version}',
        'updater.available': 'Update available: v{version}',
        'updater.upToDate': 'Up to date',
        'updater.success': 'Successfully updated to v{version}. Please restart DSH or reload profile to apply changes.',
      ...
      zh: {
        ...
        'updater.desc': '通过 npm 仓库在线检查并就地升级插件，无需 SSH 终端操作。',
        'updater.current': '当前版本: v{version}',
        'updater.available': '发现新版本: v{version}',
        'updater.upToDate': '已是最新版本',
        'updater.success': '已成功升级至 v{version}。请重启 DSH 或重载配置以应用更新。',
          ...
          React.createElement('div', {
            ...
            modalTab === 'history' && formId ? React.createElement('div', null,
              ...
              React.createElement('div', { className: 'dsh-cron-form-group' },
                ...
                React.createElement('div', {
                  style: {
                    background: 'var(--dsw-alias-bg-layer-3, color-mix(in srgb, var(--dsw-alias-label-primary, #fff) 2%, transparent))',
                ...
                selectedTaskMeta && React.createElement('div', {
                  style: {
                    background: 'var(--dsw-alias-bg-layer-3, color-mix(in srgb, var(--dsw-alias-label-primary, #fff) 3%, transparent))',
                    border: '1px solid var(--dsw-alias-border-l2, #2e2e2e)',
                    borderRadius: '8px',
                    padding: '10px 14px',
                    fontSize: '12.5px',
        ...
        settingsModalOpen && React.createElement('div', { className: 'dsh-cron-modal-overlay', onClick: () => setSettingsModalOpen(false) },
          React.createElement('div', { className: 'dsh-cron-modal dsh-cron-modal-scroll', role: 'dialog', 'aria-modal': 'true', onClick: (e) => e.stopPropagation() },
            ...
            React.createElement('form', { onSubmit: handleSaveSettings },
              ...
              React.createElement('div', { style: { background: 'var(--dsw-alias-bg-layer-3, color-mix(in srgb, var(--dsw-alias-label-primary, #fff) 2%, transparent))', border: '1px solid var(--dsw-alias-border-l2, #2e2e2e)', borderRadius: '8px', padding: '12px 14px', marginBottom: '16px' } },
