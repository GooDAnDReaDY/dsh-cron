window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Register the first slot candidate that the current DSH build accepts.
     * Direct register in try/catch: a missing mount point throws, a present
     * one renders the card; no synchronous `ok` flag guessing (#85).
     */
    function registerFirst(ctx, candidates, component) {
      ...
      for (const entry of candidates) {
        try {
          ...
          console.warn('[dsh-cron] slot registration skipped:', entry.name, '-', err && err.message);
        }
      }
      return undefined;
    }
    ...
    function apply(ctx) {
      ...
      const cardSlot = registerFirst(ctx, [
        { name: 'settings.plugin.item', key: NS, locale: NS, inject: () => ({ ctx, toggle }) },
        { name: 'settings.section', id: PLUGIN_ID, order: 34, locale: NS, label: () => translate('settings.sectionLabel'), inject: () => ({ ctx, toggle }) }
      ], CronSettingsCard);

      const chipSlot = registerFirst(ctx, [
        { name: 'conversation.session.header.utilities', id: PLUGIN_ID + '.chip', order: 27, locale: NS, inject: () => ({ ctx, toggle }) }
