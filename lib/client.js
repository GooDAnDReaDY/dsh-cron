  ...
  factory: (require) => {
    ...
    const STRINGS = {
      en: {
        ...
        'form.templateHint': 'Variables: {title} {id} {status} {output} {error} {duration} {schedule} {time} {tokens} {cost}. Empty = built-in text.',
        ...
        'settings.smtpHostLabel': 'SMTP host',
        'settings.smtpPortLabel': 'SMTP port',
        'settings.smtpUserLabel': 'SMTP user',
        'settings.smtpPasswordRefLabel': 'SMTP password credential name',
        'settings.smtpFromLabel': 'From address',
        'settings.smtpToLabel': 'Recipients (comma separated)',
        'settings.smtpSecureLabel': 'SMTP over TLS (implicit)',
        ...
        'settings.cardDesc': 'Scheduled task execution (cron/intervals), background agent scenarios, and delivery of run reports to Telegram, Discord, Slack, ntfy, Bark, PushPlus, email, voice and Gitea.',
    ...
    const DELIVERY_CHANNELS = ['telegram', 'kanban', 'discord', 'slack', 'ntfy', 'bark', 'pushplus', 'email', 'tts', 'gitea'];
    ...
    const DELIVERY_SETTING_FIELDS = [
      ...
      { key: 'smtpPasswordRef', labelKey: 'settings.smtpPasswordRefLabel', group: 'secrets' },
      ...
      { key: 'smtpHost', labelKey: 'settings.smtpHostLabel', group: 'channels' },
      { key: 'smtpPort', labelKey: 'settings.smtpPortLabel', group: 'channels' },
      { key: 'smtpUser', labelKey: 'settings.smtpUserLabel', group: 'channels' },
      { key: 'smtpFrom', labelKey: 'settings.smtpFromLabel', group: 'channels' },
      { key: 'smtpTo', labelKey: 'settings.smtpToLabel', group: 'channels' },
    ...
    const DELIVERY_TOGGLE_FIELDS = [
      { key: 'smtpSecure', labelKey: 'settings.smtpSecureLabel' },
    ];
    const DELIVERY_SETTING_KEYS = DELIVERY_SETTING_FIELDS.map((f) => f.key).concat(DELIVERY_TOGGLE_FIELDS.map((f) => f.key));
    ...
    function renderDeliverySettings({ T, values, onChange, onChannelTemplate, sections, onToggleSection }) {
      ...
      return [
        ...
        section('channels', 'settings.deliverySection', [
          ...
          ...DELIVERY_TOGGLE_FIELDS.map((f) => React.createElement('label', { className: 'dsh-cron-checkbox-row', key: f.key, style: { marginBottom: '10px' } },
            React.createElement('input', {
              type: 'checkbox',
              checked: Boolean(values[f.key]),
              onChange: (e) => onChange(f.key, e.target.checked)
            }),
            React.createElement('span', null, T(f.labelKey))
          )),
        ]),
    ...
    function CronScreen(props) {
      ...
      const updateDelivery = (key, value) => {
        setSettingsDelivery((prev) => Object.assign({}, prev, { [key]: value }));
      };
      ...
      return React.createElement('div', { className: 'dsh-cron-card' },
        ...
        cardOpen && React.createElement('div', { className: 'dsh-cron-card-body' },
          ...
          ...renderDeliverySettings({
            T: (key, vars) => T_KEY(t, key, vars),
            values: delivery,
            onChange: (key, value) => setDelivery((prev) => Object.assign({}, prev, { [key]: value })),
