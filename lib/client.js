window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function CronScreen(props) {
      ...
      React.useEffect(() => {
        fetchTasks();
        const pollInterval = setInterval(() => {          // Background polling without resetting loading spinner
    ...
    function mountSidebarJobs(toggle, label, openTaskSignal) {
      if (typeof document === 'undefined') return () => {};
      const STORAGE_KEY = 'dsh-cron-sidebar-open';
      const MAX_ROWS = 5;
      let root;
      let listEl;
      let headEl;
      let open = false;
      try { open = window.localStorage.getItem(STORAGE_KEY) === '1'; } catch (_) {}
      let fetchTimer;

      const readOpen = () => {
        try { return window.localStorage.getItem(STORAGE_KEY) === '1'; } catch (_) { return open; }
      };
      ...
      const render = (tasks) => {
        ...
        for (const task of tasks.slice(0, MAX_ROWS)) {
          ...
          const when = task.running
            ? T('list.runningNow')
            : (task.nextRunAt ? new Date(task.nextRunAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : (task.scheduleText || task.schedule || ''));
          ...
          row.addEventListener('click', (event) => {
            event.preventDefault();
            event.stopPropagation();
            openTaskSignal(task.id);
            if (!toggle.isOpen()) toggle.toggle();
          });
        ...
        if (tasks.length > MAX_ROWS) {
          ...
          more.textContent = T('sidebar.moreJobs', { count: tasks.length - MAX_ROWS });
      ...
      const load = async () => {
        try {
          ...
          const data = await res.json();
          if (data && data.ok) render(data.tasks || []);
        } catch (_) {}
      };
      ...
      const applyOpen = () => {
        ...
        if (headEl) headEl.setAttribute('aria-expanded', open ? 'true' : 'false');
        if (open) load();
      };
      ...
      const build = () => {
        ...
        headEl = document.createElement('button');
        headEl.type = 'button';
        ...
        headEl.addEventListener('click', (event) => {
          ...
          event.stopPropagation();
          open = !open;
          try { window.localStorage.setItem(STORAGE_KEY, open ? '1' : '0'); } catch (_) {}
          applyOpen();
        });
      ...
      const place = () => {
        ...
        if (root === undefined || !root.isConnected) {
          if (root === undefined) build();
          entry.insertAdjacentElement('afterend', root);
          root = document.querySelector('[data-dsh-part="sidebar-jobs"]') || root;
          // The reference may point at a detached node after re-renders.
          listEl = root.querySelector('.dsh-cron-sidebar-jobs-list');
          headEl = root.querySelector('.dsh-cron-sidebar-jobs-head');
          applyOpen();
        }
      };
      ...
      place();
      load();
      fetchTimer = setInterval(() => { if (readOpen()) load(); }, 30000);

      return () => {
        observer.disconnect();
        clearInterval(fetchTimer);
        if (root !== undefined && root.isConnected) root.remove();
        root = undefined;
      };
    }
