...
import { deliverRun, CHANNEL_IDS } from './channels.js';
...
export const Config = z.object({
  ...
  deliveryTimeoutMs: z.number().default(15000).description('Per-channel delivery timeout in ms; a slower endpoint is recorded as a failure and does not block other channels or the next tick'),
  ...
  smtpHost: z.string().default('').description('SMTP host for email notifications'),
  smtpPort: z.number().default(587).description('SMTP port'),
  smtpSecure: z.boolean().default(false).description('Use implicit TLS for SMTP'),
  smtpUser: z.string().default('').description('SMTP user name'),
  smtpPasswordRef: z.string().default('').description('Credential name for the SMTP password'),
  smtpFrom: z.string().default('').description('From address for email notifications'),
  smtpTo: z.string().default('').description('Comma-separated recipient addresses'),
...
export const SETTINGS_SYNC_KEYS = [
  ...
  'pushplusTokenRef',
  'pushplusUrl',
  'smtpHost', 'smtpPort', 'smtpSecure', 'smtpUser', 'smtpPasswordRef', 'smtpFrom', 'smtpTo',
  'ttsBaseUrl',
 ...
 *   value (bot token, SMTP password, webhook URLs, Bark key);
