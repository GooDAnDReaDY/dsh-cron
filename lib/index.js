 ...
 * configuration artefact, and secrets are never part of it because channels
...
/**
 * Validate an import document. Returns `{ ok: true, tasks }` or
 * `{ ok: false, error }`; a malformed file is rejected as a whole so a partial
 * import can never leave the store half-updated.
 */
    ...
    try {
      ...
      if (id === 'import' && req.method === 'POST') {
        ...
        for (const task of plan.add) {
          const created = store.set({ ...task, id: task.id || randomUUID(), status: task.status || 'paused', updatedAt: Date.now() });
          imported.push(created);
          if (created.status === 'active') scheduler.scheduleTask(created);
        }
        for (const task of plan.replace) {
          const replaced = store.set({ ...task, updatedAt: Date.now() });
          imported.push(replaced);
          if (replaced.status === 'active') scheduler.scheduleTask(replaced);
          else scheduler.pauseTask(replaced.id);
        }
        sendJson(res, 200, {
          ok: true,
