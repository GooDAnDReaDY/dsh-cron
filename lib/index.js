...
// The REST surface lives in lib/api.js; re-exported here so tests and tools
// keep importing it from the plugin entry point.
export { createCronApiHandler } from './api.js';
...
export const SETTINGS_SYNC_KEYS = [
  ...
  'template', 'channelTemplates', 'deliveryTimeoutMs', 'silentRuleModel',
...
export function apply(ctx, config) {
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Change an existing scheduled task. Ask the user what to change first; for tasks that run code (script/node/python/ssh/docker) confirm the change explicitly before applying it. Only the documented task fields can be changed.',
    parameters: {
      id: { type: 'string', description: 'Task identifier', required: true },
    ...
    execute: async (args) => {
      ...
      const result = applyTaskPatch({ store, scheduler, id, body: patch });
