...
export async function applySettingsToScope(settingsScope, body) {
  ...
  const errors = [];
  for (const key of ['botToken', 'chatId', 'notifyTelegram', 'onlyOnFailure', 'kanbanBaseUrl']) {
...
const createTaskParameters = {
  ...
  env: { type: 'object', description: 'Environment variables for external runtimes (KEY: value); do not store secrets here' },
  ...
  httpHeaders: { type: 'object', description: 'Headers for http tasks' },
