  ...
  return async function handleCronApi(req, res) {
    try {
      ...
      if (id === 'import' && req.method === 'POST') {
        ...
        // Every imported task is paused (the file's status is ignored), so the
        // store is only ever written with inert tasks and no scheduler arming
        // happens here. The writes are rolled back if one of them throws, so a
        // failed import cannot leave the store half-updated.
        const imported = [];
        const rollback = [];
        try {
