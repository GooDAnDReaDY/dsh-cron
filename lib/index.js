...
/** Settings keys synchronized between the store and the settings service. */
export const SETTINGS_SYNC_KEYS = [
  'botToken', 'botTokenRef', 'chatId', 'notifyTelegram', 'onlyOnFailure', 'kanbanBaseUrl',
  'template', 'channelTemplates', 'deliveryTimeoutMs',
    ...
    try {
      ...
      if (!id) {
        ...
        if (req.method === 'POST') {
          ...
          // /dsh-cron/tasks, otherwise that task could never be fetched or
      ...
      if (id === 'import' && req.method === 'POST') {
        ...
        // The import file is external input: importing code-executing tasks
        // needs the same explicit confirmation that creating one over HTTP
        // needs, otherwise a hand-edited file would bypass that gate (#86).
        ...
        if (body && body.dryRun) {
          sendJson(res, 200, {
            ...
            summary: { add: plan.add.length, replace: plan.replace.length, skip: plan.skip.length },
            codeExecuting: hasCodeExecutingTask(plan.add.concat(plan.replace)),
          });
          return;
        }
        // Every imported task is paused (the file's status is ignored) and every
        // id that replaced an existing task is disarmed: writing "paused" to the
        // store does not stop an already-armed croner job, so the scheduler must
        // be told explicitly. The writes are rolled back if one of them throws,
        // so a failed import cannot leave the store half-updated.
        const imported = [];
        const rollback = [];
        const disarm = [];
        try {
          ...
          sendJson(res, 500, { ok: false, error: `Import failed and was rolled back: ${writeErr.message}` });
          return;
        }
        ...
        for (const id of disarm) {
          try { scheduler.pauseTask(id); } catch (err) { console.error('[dsh-cron] import: could not pause task', id + ':', err.message); }
        }
        sendJson(res, 200, {
          ok: true,
