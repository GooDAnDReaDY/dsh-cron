  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        const botToken = (body.botToken || currentSettings.botToken || '').trim();
        const chatId = (body.chatId || currentSettings.chatId || '').trim();

        if (!botToken) {
          sendJson(res, 400, { ok: false, error: 'Укажите Telegram Bot Token' });
          return;
        }
        if (!chatId) {
          sendJson(res, 400, { ok: false, error: 'Укажите Telegram Chat ID' });
          return;
        }
        ...
        await sendTelegramMessage({
          botToken,
          chatId,
          text: '⏰ *DSH Cron — Проверка связи*\nТестовое уведомление успешно доставлено! Планировщик готов к отправке отчётов.',
          parseMode: 'Markdown'
        });

        sendJson(res, 200, { ok: true, message: 'Сообщение успешно отправлено в Telegram' });
      } catch (err) {
        sendJson(res, 500, { ok: false, error: err.message });
      }
    }
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        const historyMatch = pathname.match(/^\/dsh-cron\/action\/([^/]+)\/history$/);
        if (req.method === 'GET' && historyMatch) {
          const taskId = historyMatch[1];
          const history = store.getHistory(taskId, 30);
  ...
  if (ctx.tools && typeof ctx.tools.register === 'function') {
    ctx.tools.register(defineTool({
      ...
      parameters: {
        ...
        type: { type: 'string', description: 'Тип задачи: "llm" (агент с LLM) или "script" (прямой запуск shell без LLM)' },
        provider: { type: 'string', description: 'ID провайдера LLM для выполнения задачи' },
        model: { type: 'string', description: 'ID модели LLM для выполнения задачи' },
        notifyTelegram: { type: 'boolean', description: 'Отправлять отчет в Telegram' },
        onlyOnFailure: { type: 'boolean', description: 'Отправлять в Telegram только при ошибках' },
        timeoutSeconds: { type: 'integer', description: 'Таймаут выполнения задачи в секундах (по умолчанию 1800 = 30 мин)' },
        overlapPolicy: { type: 'string', description: 'Политика наложения: "skip" (по умолчанию), "queue", "replace"' }
      },
      output: {
        schema: {
          type: 'object',
      ...
      execute: async ({ title, schedule, prompt, delivery, provider, model, type, notifyTelegram, onlyOnFailure, timeoutSeconds, overlapPolicy }) => {
        try {
          ...
          const task = store.set({
            ...
            prompt,
            type: type || (provider || model ? 'llm' : 'llm'),
            delivery: delivery || 'isolated',
            provider: provider || undefined,
            model: model || undefined,
            notifyTelegram: Boolean(notifyTelegram),
            onlyOnFailure: Boolean(onlyOnFailure),
            timeoutSeconds: timeoutSeconds !== undefined ? Number(timeoutSeconds) : 1800,
