...
export const Config = z.object({
  botToken: z.string().role('secret').default('').description('Telegram Bot Token для отправки отчетов'),
  chatId: z.string().default('').description('Telegram Chat ID получателя'),
  notifyTelegram: z.boolean().default(false).description('Включить глобальную отправку отчетов в Telegram'),
  onlyOnFailure: z.boolean().default(false).description('Отправлять отчеты в Telegram только при ошибках'),
  kanbanBaseUrl: z.string().default('http://127.0.0.1:3000').description('Базовый URL Kanban API'),
});
...
export function apply(ctx, config) {
  ...
  ctx.inject(['settings'], (sctx) => {
    try {
      const scope = sctx.settings?.register?.('@goodandready/dsh-cron', Config, {
    ...
    handler: async (req, res) => {
      try {
        ...
        if (req.method === 'PATCH') {
          ...
          if (body.schedule) {
            const parsed = parseScheduleExpression(body.schedule);
            patchData.schedule = parsed.cronPattern || body.schedule;
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        if (action === 'history' || (req.method === 'GET' && !action)) {
          const limit = parseInt(url.searchParams.get('limit') || '20', 10);
          const history = store.getHistory(id, limit);
          sendJson(res, 200, { ok: true, history });
          return;
        ...
        sendJson(res, 405, { ok: false, error: 'Action not supported' });
      } catch (err) {
        sendJson(res, 500, { ok: false, error: err.message });
      }
    }
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      title: { type: 'string', description: 'Краткое название задачи', required: true },
      schedule: { type: 'string', description: 'Расписание: cron ("0 9 * * *"), интервал ("every 2h") или разово ("in 30m", "at: 2026-09-05T15:00:00Z")', required: true },
      prompt: { type: 'string', description: 'Промпт/инструкция для агента при запуске', required: true },
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Алиас для cron_create_task: создать новую запланированную задачу cron',
    parameters: {
      title: { type: 'string', description: 'Краткое название задачи', required: true },
      schedule: { type: 'string', description: 'Расписание: cron, интервал или разово', required: true },
      prompt: { type: 'string', description: 'Промпт/инструкция для агента при запуске', required: true },
      type: { type: 'string', enum: ['llm', 'prompt', 'script'], description: 'Тип задачи' },
      delivery: { type: 'string', enum: ['current', 'isolated'], description: 'Режим запуска' },
      provider: { type: 'string', description: 'Провайдер модели (опционально)' },
      model: { type: 'string', description: 'Модель (опционально)' },
      notifyTelegram: { type: 'boolean', description: 'Отправлять отчет в Telegram' },
      onlyOnFailure: { type: 'boolean', description: 'Отправлять отчет только при сбоях' },
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render: (_args, val) => [{ type: 'text', text: `Всего задач: ${val.count}` }]
    },
    execute: async (args) => {
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      id: { type: 'string', description: 'Идентификатор задачи', required: true }
    },
    output: {
      schema: {
        type: 'object',
