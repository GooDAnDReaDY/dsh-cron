...
/**
 * Shared implementation for the /dsh-cron/tasks/:id/:action and
 * /dsh-cron/action/:id/:action route families — the #66 alias is kept alive
 * by the same code path instead of a duplicated handler.
 */
function createTaskRouteHandler(store, scheduler) {
  return async function handleTaskRoute(req, res) {
    try {
      const url = new URL(req.url, 'http://127.0.0.1');
      const parts = url.pathname.split('/').filter(Boolean);
      // Both route families share the layout ['dsh-cron', 'tasks'|'action', id, action?]
      const id = parts[2];
...
export function apply(ctx, config) {
  ...
  // PATCH & DELETE /dsh-cron/tasks/:id (+ actions run/pause/resume/toggle/delete/history)
  const taskRouteHandler = createTaskRouteHandler(store, scheduler);
  ctx.effect(() => ctx.webServer.register({
    kind: 'prefix',
    path: '/dsh-cron/tasks/',
    handler: taskRouteHandler
  }), 'dsh-cron: /tasks/:id');
