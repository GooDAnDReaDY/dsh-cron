  ...
  async _run(task, signal) {
    ...
    if (this.ctx && this.ctx.agents && typeof this.ctx.agents.create === 'function') {
      try {
        ...
        // #29: code-modifying tasks may run in an isolated git worktree.
        let cwd = baseCwd;
        ...
        // #6/#10: skill- and workflow-driven tasks are agent-mediated with a
        // shaped prompt naming the skill/workflow.
        let agentPrompt = String(task.prompt || '');
        if (runtime === 'skill' && task.skillName) {
          agentPrompt = `Use the DSH skill "${task.skillName}" for this task:\n${agentPrompt}`;
        } else if (runtime === 'workflow' && task.workflowName) {
          agentPrompt = `Execute this through the DSH workflow "${task.workflowName}" step by step:\n${agentPrompt}`;
        }
        let handle = null;
        try {
          ...
          if (taskWorktree && !task.keepWorktree) {
            await this.removeTaskWorktree(baseCwd, taskWorktree);
          }
        }
      } catch (err) {
