...
export class TaskStore {
  ...
  static get FORBIDDEN_SETTING_KEYS() {
    return ['smtpPassword', 'giteaToken', 'ntfyToken', 'pushplusToken', 'discordToken', 'slackToken'];
  }

  /**
   * Settings that hold a secret and are therefore masked on read: tokens,
   * passwords, webhook URLs (the URL embeds the auth token) and the Bark key.
   * These are stored in the plugin settings file; the credential-reference
   * fields (botTokenRef, smtpPasswordRef, …) take precedence over them.
   */
  static get MASKED_SETTING_KEYS() {
    return ['botToken', 'smtpPassword', 'discordWebhookUrl', 'slackWebhookUrl', 'barkKey'];
  }
