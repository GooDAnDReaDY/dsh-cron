...
export class TaskStore {
    ...
    if (fs.existsSync(this.filePath)) {
      try {
        ...
        if (!fs.existsSync(bakPath)) {
          bestEffort('backup-initial', () => fs.copyFileSync(this.filePath, bakPath));
        }
      } catch (err) {
        console.error('[dsh-cron] store read error (corrupted file):', err.message);
        try {
          const corruptedPath = `${this.filePath}.corrupted.${Date.now()}`;
          fs.copyFileSync(this.filePath, corruptedPath);
          console.warn(`[dsh-cron] corrupted store backed up to ${corruptedPath}`);
        } catch (snapErr) {
          console.error('[dsh-cron] failed to snapshot corrupted store:', snapErr.message);
        }
        if (fs.existsSync(bakPath)) {
          try {
            const rawBak = fs.readFileSync(bakPath, 'utf-8');
            loadData(rawBak);
            console.warn(`[dsh-cron] successfully recovered tasks store from ${bakPath}`);
          } catch (bakErr) {
            console.error('[dsh-cron] failed to recover tasks store from .bak:', bakErr.message);
          }
        }
      }
      ...
      try {
        const rawBak = fs.readFileSync(bakPath, 'utf-8');
        loadData(rawBak);
        bestEffort('restore-from-bak', () => fs.copyFileSync(bakPath, this.filePath));
        console.warn(`[dsh-cron] restored missing tasks store from ${bakPath}`);
      } catch (bakErr) {
        console.error('[dsh-cron] failed to restore tasks store from .bak:', bakErr.message);
      }
    }
  }

  /**
   * Schedule a debounced save to avoid synchronous event loop blocking on high-frequency run completions.
   */
  ...
  save() {
    ...
    try {
      ...
      if (now - this._lastBackupTime > 60000) {
        ...
        bestEffort('backup-on-save', () => fs.copyFileSync(this.filePath, `${this.filePath}.bak`));
      }
    } catch (err) {
  ...
  _archiveHistoryRuns(id, overflowRuns) {
    ...
    try {
      ...
      fs.writeFileSync(tmp, JSON.stringify(archive), 'utf-8');
      fs.renameSync(tmp, archivePath);
    } catch (archiveErr) {
      console.error('[dsh-cron] history archive error:', archiveErr.message);
    }
  }
  ...
  getArchivedRuns(id, { limit = 50, offset = 0, search = '' } = {}) {
    try {
      ...
      let runs = Array.isArray(archive[id]) ? archive[id] : [];
