...
export class TaskStore {
  ...
  getSettings() {
    ...
    return {
      botToken: this.settings.botToken !== undefined ? this.settings.botToken : (defaults.botToken || ''),
      botTokenRef: this.settings.botTokenRef || '',
      chatId: this.settings.chatId !== undefined ? this.settings.chatId : (defaults.chatId || ''),
      notifyTelegram: Boolean(this.settings.notifyTelegram),
      onlyOnFailure: Boolean(this.settings.onlyOnFailure),
      hasDefaultCredentials: Boolean(defaults.botToken && defaults.chatId),
      kanbanBaseUrl: this.settings.kanbanBaseUrl || 'http://127.0.0.1:3000',
    };
  }
  ...
  getClientSettings() {
    ...
    return {
      ...
      botTokenRef: raw.botTokenRef,
      hasBotToken: hasToken,
      hasCustomBotToken: isCustom,
      chatId: raw.chatId,
      notifyTelegram: raw.notifyTelegram,
      onlyOnFailure: raw.onlyOnFailure,
      hasDefaultCredentials: raw.hasDefaultCredentials,
      kanbanBaseUrl: raw.kanbanBaseUrl,
    };
  }
  ...
  saveSettings(newSettings = {}) {
    ...
    if (newSettings.botToken !== undefined) {
      const val = String(newSettings.botToken).trim();
    ...
    this.settings = {
      ...
      botTokenRef: newSettings.botTokenRef !== undefined
        ? String(newSettings.botTokenRef).trim()
        : (this.settings.botTokenRef || ''),
      chatId: newSettings.chatId !== undefined ? String(newSettings.chatId).trim() : (this.settings.chatId || ''),
      notifyTelegram: newSettings.notifyTelegram !== undefined ? Boolean(newSettings.notifyTelegram) : Boolean(this.settings.notifyTelegram),
      onlyOnFailure: newSettings.onlyOnFailure !== undefined ? Boolean(newSettings.onlyOnFailure) : Boolean(this.settings.onlyOnFailure),
      kanbanBaseUrl: newSettings.kanbanBaseUrl !== undefined ? String(newSettings.kanbanBaseUrl).trim() : (this.settings.kanbanBaseUrl || 'http://127.0.0.1:3000'),
    };
    this.save();
