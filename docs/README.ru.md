...
* алиасы `daily` / `hourly` / `weekdays` (`weekdays` → `0 9 * * 1-5`)
...
* Задачи выполняются в **локальном часовом поясе сервера**; cron-выражения вычисляет `croner` по часам хоста.
...
| `POST` | `/dsh-cron/settings` | Обновление настроек интеграций |
