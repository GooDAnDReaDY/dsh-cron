...
- 2026-09-13 — Мониторинг тишины, предварительные проверки, самовосстановление и UX Pack (0.2.11, #139): Heartbeat / Dead Man's Snitch эндпоинты /dsh-cron/heartbeat/:id и /dsh-cron/api/heartbeat/:id с отслеживанием дедлайна и onFailure алертом; Pre-flight проверки (HTTP 2xx, shell exit code, disk space MB) с пропуском холостых вызовов без траты LLM токенов; режим Dry-Run (POST /dsh-cron/tasks/:id/dry-run) для тестового запуска без записи в историю и без отправки в каналы; симулятор расписания previewSchedule (POST /dsh-cron/schedule/preview); приоритетные очереди исполнения при конкурентном лимите; автоматические команды самоисцеления selfHealingCommand и авто-диагностика сбоев autoDiagnose; Web UI архив логов с пагинацией и визуализацией цепочек задач.
- 2026-09-16 — Декомпозиция клиентской части и темизация (#149, #150): lib/client.js разбит на модульные фрагменты lib/client-src/*.js (каждый <= 580 строк) со сборкой через scripts/build-client.mjs (pretest hook). Инлайн-стили тегов onSuccess/onFailure/heartbeat/targetSession/preflight и стили оверлея переведены на семантические CSS-переменные темы (--dsh-cron-*, var(--dsw-alias-bg-mask, rgba(0, 0, 0, 0.75))). Фрагменты исходников исключены из npm-пакета через files: ["lib/*.js", ...].
